...
$(document).ready(function() {
  ...
  if( typeof isListPage  !== 'undefined' && isListPage  === '1' ){
    ...
    setDataTable("casestudyTable", tableColumns);
    ...
    $(".deleteBtn").on("click", function(e){
      ...
      deleteRecord(routes.CASESTUDY_DELETE, formdata);
    });
  }
  ...
  if( typeof isFormPage  !== 'undefined' && isFormPage === '1' ){
    ...
    if( typeof isEditFormPage  !== 'undefined' && isEditFormPage === '1' ){
      $("#casestudyUpdateForm").on("submit",function(e){
        e.preventDefault();        
    
        const formData = new FormData(this);
        postForm(routes.CASESTUDY_UPDATE, formData);
      });
    }else{
      $("#casestudyAddForm").on("submit", function(e){
        e.preventDefault();        
    
        const formData = new FormData(this);
        postForm(routes.CASESTUDY_ADD, formData);
      });
    }
